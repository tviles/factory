...
Reusable combination of **agents plus code**: deterministic Python ADW scripts own sequencing, retries, and acceptance; coding agents work inside bounded phases; typed JSON envelopes carry context between them; everything streams into SQLite for the polled visualizer. Agent proposes, code disposes.
...
The default roster still runs `coding_agent: pi`, default model
`gemini-3.6-flash` via openrouter, thinking `medium`. The visualizer app ships
in a later pass — observe via sqlite queries until then.
