...
For pi there is no separate continue call to make: `agent_pi.run()` passes `--session-id`, which pi treats as create-or-continue, so running an agent and continuing it are the same call with the same id. Claude Code's adapter needs one more bit of state instead — `--session-id` CREATES and fails if the id exists, `--resume` continues — so `agents.py` picks the flag based on whether the session was already in `agent_map.json`; either way, context stays intact across a retry. Before parsing, the harness also tolerates a fenced `json` code block or prose wrapped around the object — but the prompt still asks for bare JSON, and every failed attempt is persisted as an invalid envelope row.
